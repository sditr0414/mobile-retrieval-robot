...
- 전방 초음파 센서로 안전거리를 확인한 뒤에만 로봇팔 전개 허용
...
- 휠 엔코더는 사용하지 않으며 정밀 위치·거리 PID는 구현 대상에서 제외
...
로봇팔 전개 명령은 아래 조건을 모두 만족할 때만 실행합니다.
...
초음파 센서는 좁은 물체, 흡음 재질, 기울어진 면에서 오측정할 수 있으므로 단일 측정값만으로 전개하지 않습니다. 이미 팔이 전개된 상태에서 장애물이 가까워지면 급격히 접지 말고, 차량 정지와 경고를 우선 수행합니다.

> 20 cm는 현재 프로젝트의 초기 설정값입니다. 실제 전개 시 로봇팔이나 그리퍼가 장애물에 닿는다면 실측 결과에 따라 값을 높여야 합니다.
...
- 로봇팔 전개 전 차량 정지와 전방 안전거리 조건을 함께 확인합니다.
- 로봇팔을 펼친 상태에서는 차량 최고 속도를 제한합니다.
- 초음파 센서가 응답하지 않거나 비정상 값을 내면 전개를 금지합니다.
