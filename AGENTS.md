...
- 홈/주행 준비 자세는 Base 0도, Shoulder -50도, Elbow +90도,
  Wrist Tilt 0도, Wrist Rotate +60도, Gripper 0%이며 패킷값은
  `[90,40,180,90,150,0]`
- 로봇팔 이동 또는 티칭 재생을 시작하기 전에 차량을 즉시 정지
- 관절 이동과 웨이포인트 유지가 끝날 때까지 일반 주행 명령을 적용하지 않음
...
- 앱 아래쪽의 3개 제어 탭으로 RC카, 로봇팔, 티칭 화면을 바로 전환
- 로봇팔과 티칭 화면은 앱 공용 웨이포인트 상태를 사용하며 앱 실행 중 유지
- 로봇팔 화면에서는 시퀀스 선택과 현재 자세의 웨이포인트 저장·삭제만 제공
- STM32 Flash 업로드·재생·초기화는 티칭 시스템 화면에서만 제공
...
| `telemetryTask` | Low | 768 words | 100 ms 주기 USART2 Teleplot 출력 |
...
- Base, Elbow, Wrist Tilt와 Gripper는 조립 방향에 맞춰 출력 방향을 반전
- 홈/주행 준비 자세는 Base `0도`, Shoulder `-50도`, Elbow `+90도`,
  Wrist Tilt `0도`, Wrist Rotate `+60도`, Gripper `0%`이며 패킷값은
  `[90,40,180,90,150,0]`
- 일반 관절은 `1도/20 ms`, 약 `50도/s`로 속도를 제한
- Gripper는 원래 속도인 패킷 `1단계/15 ms`로 이동
- USART2 115200 baud에서 Teleplot 형식으로 현재값, 목표값, 펄스폭과 상태 출력
- Teleplot UART 전송은 저우선순위 `telemetryTask`에서 수행
- Teleplot의 현재값은 위치 센서 실측값이 아니라 마지막 PWM 출력 명령값
...
- 로봇팔 이동과 티칭 재생 중에는 차량 PWM을 0으로 유지
- 주행 준비 자세로 복귀 중인 경우에도 자세 도달 전까지 주행 명령을 적용하지 않음
