...
- Base, Elbow와 Wrist Tilt는 조립 방향 때문에 출력 방향을 반전합니다.
- Gripper도 출력 방향을 반전해 `0%=1840 us`,
  `100%=1140 us`로 출력합니다.
- Data6은 `0` 이동, `1` 지정 자세로 출력 활성화, `2` 출력 차단입니다.
- 부팅 직후에는 서보 출력을 끄며, 활성화 명령 전에는 이동을 거부합니다.
- 앱의 홈/주행 준비 자세는 Base `0도`, Shoulder `-50도`, Elbow `+90도`,
  Wrist Tilt `0도`, Wrist Rotate `+60도`, Gripper `0%`이며 패킷값은
  `[90,40,180,90,150,0]`입니다.
- 로봇팔 출력이 활성화된 상태에서 RC카 화면으로 이동하면 앱은 같은 준비
  자세를 요청하며, 차량 주행 중에도 서보 출력은 유지됩니다.
- 로봇팔 이동 명령이나 티칭 재생을 받으면 펌웨어가 차량을 즉시 정지하고,
  관절 이동과 웨이포인트 유지가 끝날 때까지 일반 주행 명령을 폐기합니다.
- RC카 화면으로 이동할 때 준비 자세가 아직 이동 중이면 차량은 정지 상태를
  유지하고, 자세 도달 후 반복해서 들어오는 주행 패킷부터 적용합니다.
- 일반 관절은 최대 `1도/20 ms`, 약 `50도/s`로 보간합니다.
- Gripper는 원래 속도인 패킷 `1단계/15 ms`로 보간합니다.
...
예: 홈/주행 준비 자세로 서보 출력 활성화

```text
AA 01 5A 28 B4 5A 96 00 01 28 55
```
