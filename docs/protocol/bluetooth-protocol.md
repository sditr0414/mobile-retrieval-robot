...
앱은 PID 보정 여유를 남기기 위해 조이스틱 출력을 최대 `220 PWM`으로 제한합니다.
`PidStraight`는 앱의 X축 직진 판정 범위로 결정하며 기본값은 `0.10`, PID 유지
중에는 `+0.04` 히스테리시스를 사용합니다. 이 값은 앱 내부 설정이므로 패킷과
STM32 Flash에는 포함하지 않습니다.
...
- 앱은 성공 응답 뒤 저장 시퀀스의 마지막 웨이포인트를 수동 제어 화면에 반영.
- 앱은 마지막 웨이포인트 조회부터 작업을 잠그고 조회 실패 시 재생 요청을 보내지 않음.
...
앱은 성공 응답 뒤 편집본의 마지막 웨이포인트를 수동 제어 화면에 반영합니다.
