...
| 기능 | STM32 핀 | 연결 |
|---|---|---|
| I2C1 SCL | PB8 | PCA9685 SCL, MPU6050 SCL |
| I2C1 SDA | PB9 | PCA9685 SDA, MPU6050 SDA |
...
| `i2cMutex` | Mutex | PCA9685와 MPU6050 버스 공유 |
...
- 무게중심, 관성텐서와 충돌 형상은 근사값.
- 기본 중력 0.
- 토크, 처짐, 충돌 안전과 실물 속도를 보증하지 않음.
- 차량 동역학 미구현.
- 프로젝트 최종 범위에서 미완성 보조 도구로 유지.
...
- 서보 위치 피드백이 없어 실제 도착을 직접 측정하지 못함.
- MPU6050에 자력계가 없어 절대 Yaw와 장기 drift 보정 불가.
- L298N과 DC 모터에 엔코더가 없어 실제 바퀴 속도 측정 불가.
- Flash 저장 이미지가 단일 복사본이라 기록 중 전원 장애 복구가 제한적.
- 물리 전원 차단 장치, BMS와 퓨즈가 없음.
- 시뮬레이터가 실물 동역학을 보증하지 않음.
