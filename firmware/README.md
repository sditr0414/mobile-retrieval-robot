...
로봇팔은 20 ms 5차 S-curve와 앱 속도 `50~100%`를 사용합니다. `100%`의
각속도 상한은 약 `62.5°/s`입니다. 이동 시간은 20 ms부터 25%씩 늘리며 모든
관절의 속도·가속도·저크 제한을 만족하는 가장 짧은 후보로 결정합니다. 원점은
`[90,90,90,90,90,0]`, 이동 자세는 티칭 시퀀스 9이며 현재 Gripper를
유지합니다. 위치 피드백이 없어 첫 활성화의 실제 시작 위치와 물리 속도는
보장하지 않습니다.
...
- 링커 FLASH: 384 KiB.
- 현재 버전 저장 이미지는 2,516바이트이며 기록 전 Sector 7 전체를 삭제.
